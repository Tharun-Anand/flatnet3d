...
- Updated for PyTorch 1.3 and Python 3.6 (earlier versions unsupported)s
...
for i in range(len(psf_depths)):
    single_psf = Image.open('/home/sushanth/psf_captures/'+'psf_'+str(psf_depths[i])+'.png')
...
def get_laplacian(device):
    reg, _ = uft.laplacian(2, [759, 1006], is_real=False)
...
def wiener_deconvolution(x,psf,Gamma=0):

        # Assuming your original tensor is named 'input_tensor'
        #input_tensor = self.reg_filter

        # Define the desired target size
        #target_size = (1, 12, 3, 759, 1006)

        # Calculate the required padding values for the last two dimensions
        #padding_cols = target_size[-1] - input_tensor.size(-1)
        #padding_rows = target_size[-2] - input_tensor.size(-2)

        # Perform padding using torch.nn.functional.pad
        #reg_padded = torch.nn.functional.pad(input_tensor, (0, padding_cols, 0, padding_rows))
        #Gamma=self.lambd*reg_padded
        ...
        out = F.relu((torch.fft.ifft2(outf)).real)
...
class WienerNet(torch.nn.Module):
    def __init__(self, psf, device, batch_size, fixed=False):
        super(WienerNet, self).__init__()
        self.psfs = torch.tensor(psf.transpose(2, 3, 0, 1)).to(device) 
        self.stack_size = 12
        self.device = device
        self.batch_size = batch_size

        if fixed:
            self.reg_filter = torch.tensor(regf_2).unsqueeze(0).repeat(12, 1, 1).to(device)
            self.lambd = torch.from_numpy(np.load('lambda.npy')).to(device)
#             self.lambd = 100*torch.ones([self.stack_size, 3]).to(device)
        else:
            self.reg_filter = torch.nn.Parameter(torch.Tensor([[0,-1,0],[-1,4,-1],[0,-1,0]]).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(1,12,3,1,1))
            self.lambd = torch.nn.Parameter(1e-5*torch.ones([1,12,1,1,1]))
    ...
    def forward(self, meas):
            ...
            psfs = self.psfs.unsqueeze(0)#1*3*12*759*1006
            meas = torch.tensor(meas).to(self.device)#B*3*759*1006
            image_stack = torch.zeros(meas.shape[0], 12, 3,759,1006).to(self.device)
            input_tensor = self.reg_filter
            
            target_size = (meas.shape[0], 12, 3,759,1006)

            padding_cols = target_size[-1] - input_tensor.size(-1)
            padding_rows = target_size[-2] - input_tensor.size(-2)

            reg_padded = torch.nn.functional.pad(input_tensor, (0, padding_cols, 0, padding_rows))
            reg_fft = torch.fft.fft2(torch.fft.fftshift(reg_padded,(-2,-1)))
            
            gamma = self.lambd * torch.abs(reg_fft)**2
            gamma=gamma.to(self.device)
            for j in range(12):
                image_stack[:, j, :, :, :] = wiener_deconvolution(meas, psfs[:, :, j, :, :], gamma[:,j,:,:,:])
            image_stack=image_stack.to(self.device)
            
            with torch.no_grad():
             image_stack_max = image_stack.reshape(image_stack.size(0), -1).max(1)[0].reshape(-1, 1, 1, 1, 1)
            #np.save('gamma.npy', gamma.detach().cpu().numpy())

            
            return image_stack/image_stack_max
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.in_layer=='filter':
            ...
            x = self.wiener_model(x).type(torch.float32)#B*12,3,759,1006
            ...
            x=center_crop(x,[540,540])#B,12*3,540,540
            x=F.interpolate(x, [180,180], mode='bilinear', align_corners=True)#B,12*3,180,180
            x=F.pad(x,(6,6,6,6))#B,12*3,192,192
